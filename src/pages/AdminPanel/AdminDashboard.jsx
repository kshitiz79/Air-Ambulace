...
import { EnquiryCard } from './EnquiryManagementPage';
...
const AdminDashboard = () => {
  ...
  const [data, setData] = useState({
    ...
    recent: [], completedList: [], recentUsers: [], monthly: [], userRoles: [], hospitalStats: [],
  });
  ...
  const fetchData = async () => {
    try {
      ...
      setData({
        ...
        completedList: enquiries.filter(e => e.status === 'COMPLETED').sort((a, b) => new Date(b.created_at) - new Date(a.created_at)),
  ...
  return (
    <div className={`max-w-7xl mx-auto p-6 space-y-6 ${styles.pageBackground}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
        ...
        <GradientStatCard label={t.completedCases || 'Completed Cases'} value={data.completed} icon={<FaCheckCircle />} from="from-gray-500" to="to-gray-600" onClick={() => navigate('/admin/completed-cases')} />
      </div>
      ...
      <div className={`${styles.cardBackground} rounded-lg ${styles.cardShadow}`}>
        <div className={`px-6 py-4 border-b ${styles.borderColor} flex items-center justify-between`}>
          ...
          <Link
            to="/admin/enquiry-management"
            className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition text-sm font-bold"
          >
            View All →
          </Link>
        </div>
          ...
          {filteredRecent.length === 0 ? (
            ...
            filteredRecent.map(e => (
              <EnquiryCard
                ...
                enquiry={e}
                onDelete={() => {}}
                onDetail={() => navigate(`/admin/enquiry-detail/${e.enquiry_id}`)}
              />
            ))
          )}
        </div>
      ...
      {data.completed > 0 && (
        <div className={`${styles.cardBackground} rounded-lg ${styles.cardShadow}`}>
          <div className={`px-6 py-4 border-b ${styles.borderColor} flex items-center justify-between`}>
            <h2 className={`text-xl font-semibold ${styles.primaryText} flex items-center gap-2`}>
              <span className="w-8 h-8 bg-gray-600 rounded-lg flex items-center justify-center text-white text-sm">🏁</span>
              Completed Cases
              <span className="ml-1 px-2.5 py-0.5 bg-gray-100 text-gray-700 text-sm font-black rounded-full border border-gray-200">
                {data.completed}
              </span>
            </h2>
            <Link
              to="/admin/enquiry-management"
              className="flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition text-sm font-bold"
            >
              View All →
            </Link>
          </div>
          <div className="p-4 space-y-3">
            {data.completedList.length === 0 ? (
              <div className={`p-6 text-center ${styles.secondaryText}`}>
                <p className="text-sm">No completed cases found.</p>
              </div>
            ) : (
              data.completedList.map(e => (
                <EnquiryCard
                  key={e.enquiry_id}
                  enquiry={e}
                  onDelete={() => {}}
