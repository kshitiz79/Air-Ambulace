...
const BeneficiaryDetailPage = () => {
  ...
  const labels = {
    en: {
      ...
      documents: 'Documents',
      download: 'Download',
      noIdentity: 'No identity card information available',
      years: 'years',
    },
    hi: {
      ...
      phone: 'फोन',
      email: 'ईमेल',
      hospitalDetails: 'अस्पताल के विवरण',
      ...
      documents: 'दस्तावेज़',
      download: 'डाउनलोड',
      noIdentity: 'कोई पहचान कार्ड जानकारी उपलब्ध नहीं है',
      years: 'वर्ष',
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                {labels[language].backToDashboard}
              </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].patientName}</label>
                    <p className="text-lg font-medium text-gray-900">{enquiry?.patient_name}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].fatherSpouseName}</label>
                    <p className="text-lg text-gray-900">{enquiry?.father_spouse_name}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].age}</label>
                    <p className="text-lg text-gray-900">{enquiry?.age} {labels[language].years}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].gender}</label>
                    <p className="text-lg text-gray-900">{enquiry?.gender}</p>
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].address}</label>
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {enquiry?.aadhar_card_number && (
                    <div>
                      <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].aadharCard}</label>
                      <p className="text-lg text-gray-900 font-mono">{enquiry.aadhar_card_number}</p>
                    </div>
                  )}
                  {enquiry?.pan_card_number && (
                    <div>
                      <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].panCard}</label>
                  ...
                  {!enquiry?.ayushman_card_number && !enquiry?.aadhar_card_number && !enquiry?.pan_card_number && (
                    <div className="md:col-span-2">
                      <p className="text-gray-500 italic">{labels[language].noIdentity}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border">
              ...
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].medicalCondition}</label>
                  <p className="text-gray-900">{enquiry?.medical_condition}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].chiefComplaint}</label>
                  <p className="text-gray-900">{enquiry?.chief_complaint}</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].generalCondition}</label>
                    <p className="text-gray-900">{enquiry?.general_condition}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].vitals}</label>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-sm border">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center">
                  ...
                  {labels[language].contactDetails}
                </h2>
              </div>
              <div className="p-6 space-y-4">
                <div>
            ...
            <div className="bg-white rounded-lg shadow-sm border">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-semibold text-gray-800 flex items-center">
                  ...
                  {labels[language].hospitalDetails}
                </h2>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].destinationHospital}</label>
                  <p className="text-gray-900">{enquiry?.hospital?.name || enquiry?.hospital?.hospital_name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].sourceHospital}</label>
                  <p className="text-gray-900">{enquiry?.sourceHospital?.name || enquiry?.sourceHospital?.hospital_name}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].district}</label>
            ...
            <div className="bg-white rounded-lg shadow-sm border">
              ...
              <div className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].created}</label>
                  <p className="text-gray-900">{new Date(enquiry?.created_at).toLocaleString()}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-1">{labels[language].lastUpdated}</label>
