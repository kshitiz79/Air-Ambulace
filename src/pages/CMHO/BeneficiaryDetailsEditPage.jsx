...
const labels = {
  ...
  hi: {
    title: 'रोगी के विवरण संपादित करें',
    backToList: 'सूची में वापस जाएं',
    save: 'परिवर्तन सहेजें',
    cancel: 'रद्द करें',
    loading: 'रोगी के विवरण लोड हो रहे हैं...',
    saving: 'परिवर्तन सहेजे जा रहे हैं...',
    error: 'त्रुटि: ',
    updateSuccess: 'रोगी को सफलतापूर्वक अपडेट किया गया!',
    updateError: 'रोगी को अपडेट करने में विफल: ',
    toggleLang: 'English',
...
const BeneficiaryDetailsEditPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Store raw enquiry (with _hi fields) for language switching
        setRawEnquiry(enquiry);
  ...
  // Re-localize displayed fields when language toggles
  useEffect(() => {
    if (!rawEnquiry) return;
    const loc = localize(rawEnquiry);
    setFormData(prev => ({
      ...prev,
      patient_name: loc.patient_name || '',
      father_spouse_name: loc.father_spouse_name || '',
      address: loc.address || '',
      medical_condition: loc.medical_condition || '',
      chief_complaint: loc.chief_complaint || '',
      general_condition: loc.general_condition || '',
      referral_note: loc.referral_note || '',
      recommending_authority_name: loc.recommending_authority_name || '',
      recommending_authority_designation: loc.recommending_authority_designation || '',
      approval_authority_name: loc.approval_authority_name || '',
      approval_authority_designation: loc.approval_authority_designation || '',
      referring_physician_name: loc.referring_physician_name || '',
      referring_physician_designation: loc.referring_physician_designation || '',
      medical_team_note: loc.medical_team_note || '',
      remarks: loc.remarks || '',
  ...
  const validateForm = () => {
    ...
    if (!formData.patient_name.trim()) errs.patient_name = `${labels[language].patientName} ${labels[language].required}`;
    if (!formData.father_spouse_name.trim()) errs.father_spouse_name = `${labels[language].fatherSpouseName} ${labels[language].required}`;
    if (!formData.age || formData.age <= 0) errs.age = labels[language].ageError;
    if (!formData.gender) errs.gender = `${labels[language].gender} ${labels[language].required}`;
    if (!formData.address.trim()) errs.address = `${labels[language].address} ${labels[language].required}`;

    // Identity card validation
    if (!formData.identity_card_type) {
      if (!formData.aadhar_card_number || !formData.pan_card_number) {
        errs.identity = labels[language].identityFallback;
      }
    } else if (formData.identity_card_type === 'ABHA') {
      if (!formData.ayushman_card_number) {
        errs.ayushman_card_number = `${labels[language].abhaNumber} ${labels[language].required}`;
      ...
      if (!formData.ayushman_card_number) {
        errs.ayushman_card_number = `${labels[language].pmJayId} ${labels[language].required}`;
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
                  <option value="">{labels[language].selectIdentityType}</option>
              ...
              {formData.identity_card_type && (
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder={formData.identity_card_type === 'ABHA' ? labels[language].aadharPlaceholder : 'Enter 9-digit PM JAY ID'}
              ...
              {!formData.identity_card_type && (
                <div className="space-y-4">
                  <p className="text-sm text-gray-600">{labels[language].orProvideBothAadharPan}</p>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      placeholder={labels[language].aadharPlaceholder}
                      maxLength={12}
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={labels[language].panPlaceholder}
                      maxLength={10}
                      className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
