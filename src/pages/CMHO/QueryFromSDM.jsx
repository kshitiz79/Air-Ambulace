...
const QueryFromSDM = () => {
  ...
  const fetchQueries = async () => {
    ...
    try {
      ...
      // Filter only queries raised by SDM
      const sdmQueries = (Array.isArray(queryData) ? queryData : []).filter(
        q => q.raisedBy?.role === 'SDM'
      );
      setQueries(sdmQueries);
      
      // Show info if data is filtered for CMHO users
      if (response.data.filtered && localStorage.getItem('role') === 'CMHO') {
        console.log(`Query from SDM: Showing ${queryData.length} queries related to enquiries created by current CMHO user`);
      }
    } catch (err) {
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">{t.queryFromSdm || 'Query from SDM'}</h1>
            ...
            <div className="space-y-6">
              {queries.map((query) => (
                <div key={query.query_id} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  {/* Query Content */}
                  <div className="mb-4">
                    <h4 className="font-medium text-gray-900 mb-2">{t.queryFromSdm || 'Query from SDM'}:</h4>
                    <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded">
                      <p className="text-gray-800">{query.query_text}</p>
                      <div className="flex items-center mt-3 text-sm text-gray-600">
                        <FiUser className="mr-1" />
                        <span>{t.by}: {query.raisedBy?.full_name || 'SDM'}</span>
