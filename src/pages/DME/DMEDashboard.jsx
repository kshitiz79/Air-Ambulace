...
import {
  ...
  FaDollarSign, FaSyncAlt, FaUserTie, FaPaperPlane,
} from 'react-icons/fa';
...
const DMEDashboard = () => {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    financialSanctions: 0, ordersReleased: 0, recentEnquiries: [],
    monthlyStats: [], financialStats: [],
  });
  ...
  const quickActions = [
    { title: t.approvalRejection || 'Approval/Rejection', description: t.reviewAndApproveCases || 'Review and approve cases', icon: <FaUserTie className="text-blue-600" />, to: '/dme-dashboard/approval-reject', color: 'border-blue-200 hover:border-blue-400 hover:bg-blue-50' },
    { title: t.caseFiles || 'Case Files', description: t.viewCaseFiles || 'View case files', icon: <FaFileAlt className="text-green-600" />, to: '/dme-dashboard/case-files', color: 'border-green-200 hover:border-green-400 hover:bg-green-50' },
    { title: t.financialSanctions || 'Financial Sanctions', description: t.manageFinancialApprovals || 'Manage financial approvals', icon: <FaDollarSign className="text-yellow-600" />, to: '/dme-dashboard/financial-page', color: 'border-yellow-200 hover:border-yellow-400 hover:bg-yellow-50' },
    { title: t.orderRelease || 'Order Release', description: t.releaseApprovedOrders || 'Release approved orders', icon: <FaPaperPlane className="text-purple-600" />, to: '/dme-dashboard/order-release-page', color: 'border-purple-200 hover:border-purple-400 hover:bg-purple-50' },
  ];
  ...
  const calcMonthly = (enquiries) => {
    ...
    return Array.from({ length: 6 }, (_, i) => {
      ...
      return { month, total: m.length, approved: m.filter(e => e.status === 'APPROVED').length, rejected: m.filter(e => e.status === 'REJECTED').length, pending: m.filter(e => ['PENDING','FORWARDED'].includes(e.status)).length };
    });
  };
  ...
  if (loading) return <div className="max-w-7xl mx-auto p-6 text-center text-gray-600">{t.loadingQueries || 'Loading dashboard...'}</div>;
  if (error) return (
    <div className="max-w-7xl mx-auto p-6">
      <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
        <p>{error}</p>
        <button onClick={() => { setRefreshing(true); fetchDashboardData(); }} className="mt-2 bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">{t.retry || 'Retry'}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <DashboardHeader title="DME Dashboard" badgeLabel="Director of Medical Education" onRefresh={() => { setRefreshing(true); fetchDashboardData(); }} refreshing={refreshing} accentColor="teal" />
      ...
      <CasesTable
        cases={filteredCases}
        baseRoute="/dme-dashboard"
        approvalRoute="/dme-dashboard/approval-reject"
        accentColor="teal"
        filter={filter}
        onFilterChange={e => setFilter({ ...filter, status: e.target.value })}
