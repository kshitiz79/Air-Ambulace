...
const ResponseFromCMHO = () => {
  ...
  const fetchQueries = async () => {
    ...
    try {
      ...
      // Filter to show only queries that have responses from CMHO
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Responses from CMHO</h1>
      </div>
          ...
          {queries.length > 0 ? (
            <div className="space-y-6">
              {queries.map((query) => (
                <div key={query.query_id} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  {/* CMHO Response */}
                  <div className="mb-4">
                    <h4 className="font-medium text-gray-900 mb-2">CMHO Response:</h4>
                    <div className="bg-green-50 border-l-4 border-green-400 p-4 rounded">
                      <p className="text-gray-800">{query.response_text}</p>
                      <div className="flex items-center mt-3 text-sm text-gray-600">
                        <FiUser className="mr-1" />
                        <span>By: {query.respondedBy?.full_name || 'CMHO'}</span>
                        <FiClock className="ml-4 mr-1" />
            ...
            <div className="text-center py-12">
              ...
              <div className="text-gray-500 text-lg mb-4">
                No responses from CMHO found
              </div>
              <p className="text-gray-400">
                Responses will appear here once CMHO replies to your queries
              </p>
            </div>
          )}
        </>
...
export default ResponseFromCMHO;