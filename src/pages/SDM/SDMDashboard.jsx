...
import {
  ...
  FaSearch, FaEye, FaEdit, FaArrowRight, FaExclamationTriangle,
} from 'react-icons/fa';
...
const SDMDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [enqRes, queriesRes] = await Promise.all([
        fetch(`${baseUrl}/api/enquiries`, { headers: h }),
        fetch(`${baseUrl}/api/case-queries`, { headers: h }).catch(() => ({ ok: false })),
      ]);
      if (!enqRes.ok) throw new Error('Failed to fetch enquiries');
      const [enqData, queriesData] = await Promise.all([
        enqRes.json(),
  ...
  const statusChartData = {
    labels: [t.pending || 'Pending', t.approved || 'Approved', t.rejected || 'Rejected', t.forwarded || 'Forwarded'],
    datasets: [{ data: [data.pending, data.approved, data.rejected, data.forwarded], backgroundColor: ['#fbbf24', '#10b981', '#ef4444', '#3b82f6'], borderWidth: 2 }],
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <DashboardHeader
        title={t.sdmDashboard || 'SDM Dashboard'}
        badgeLabel={t.subDivisionalMagistrate || 'Sub Divisional Magistrate'}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <GradientStatCard label={t.totalEnquiries || 'Total Enquiries'} value={data.total} subLabel={t.allTimeTotal || 'All time total'} icon={<FaFileAlt />} from="from-blue-500" to="to-blue-600" />
        <GradientStatCard label={t.pendingReview || 'Pending Review'} value={data.pending} subLabel={t.awaitingAction || 'Awaiting action'} icon={<FaClock />} from="from-yellow-500" to="to-yellow-600" />
      ...
      <CasesTable
        cases={filteredCases}
        baseRoute="/sdm-dashboard"
        approvalRoute="/sdm-dashboard/enquiry-detail-page"
        viewAllRoute="/sdm-dashboard/enquiry-detail-page"
        accentColor="blue"
        filter={filter}
